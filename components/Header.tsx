  ...
  return (
    ...
    >
      <Container className="h-full flex items-center justify-between">
        ...
        <a
          href="#hero"
          className="inline-flex items-baseline gap-[12px] group focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-cream/40 py-1"
        >
          ...
          <nav className="flex items-center gap-[44px]" aria-label="Main Navigation">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="font-ui text-[12px] font-medium uppercase tracking-[0.16em] text-cream-dim hover:text-cream transition-colors duration-200 py-1 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-cream/40"
              >
                {link.label}
              </a>
            ))}
